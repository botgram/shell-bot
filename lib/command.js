...
function Command(reply, context, command) {
  ...
  this.renderer = new renderer.Renderer(reply, this.state, {
    cursorString: "\uD83D\uDD38",
    cursorBlinkString: "\uD83D\uDD38",
    silent: context.silent, //FIXME: remember to do !!options.silent
  ...
  this.initialMessage = new util.EditedMessage(reply, this._renderInitial(), "HTML");

  // Process command output
  this.pty.on("readable", this._ptyReadable.bind(this));
...
Command.prototype._renderInitial = function _renderInitial() {
  var content = "", title = this.state.getMeta("title");
  if (title) content += escapeHtml(title) + "\n";
  content += "<strong>$ " + escapeHtml(this.command) + "</strong>";
  return content;
}

Command.prototype._ptyReadable = function _ptyReadable() {
...
Command.prototype._exit = function _exit(code, signal) {
  ...
  if (signal)
    exitedText = "\uD83D\uDC80 <strong>Killed</strong> by "+signal+".";
  else if (code === 0)
