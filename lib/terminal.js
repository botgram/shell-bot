...
util.inherits(TermState, require("events"));
...
TermState.prototype.removeChar = function removeChar(position, n) {
  var x = position[0], line = this.getLine(position[1]);
...
TermState.prototype.eraseDisplay = function eraseDisplay(n) {
  switch (n || 0) {
    case "below":
    case "after":
    case 0:
...
TermState.prototype.removeLine = function removeLine(n, y) {
  ...
  if (y > this.lines.length) return this;
  ...
  this.lines = this.lines.splice(y, n);
  return this;
};
...
TermState.prototype.insertLine = function insertLine(n, y) {
  ...
  this.getLine(y);
  this.removeLine((this.lines.length + n) - this.rows, y);
  while (n--) this.lines.splice(y, 0, { str: "", attr: null });
...
TermState.prototype.write = function write(chunk) {
  ...
  while (chunk) {
    ...
    // Flush current chunk
    if (i > 0) {
      if (this.getMode("graphics")) chunk = chunk.split("").map(function (c) {
        if (c in GRAPHICS) c = GRAPHICS[c];
        return c;
      }).join("");
      this._writeChunk(this.cursor, chunk, this.getMode("insert"));
      this.cursor[0] += chunk.length;
      this.emit("cursor");
    }
    ...
    // Process character
    if (c == '\n') {
      if (this.lines.length + 1 > this.rows)
        this.scroll(1);
      this.mvCursor(0, 1);
      this.getLine();
    }
...
TermState.prototype.mvCursor = function mvCursor(x, y) {
  ...
  return this.setCursor(cursor[0] + x, cursor[0] + y);
};
...
//TODO: verify accesses to this.cursor
