...
function parseSocksUrl(proxy) {
  var proxy_str = proxy.replace("socks5://", "");

  var proxy_obj = {}

  proxy_obj.socksHost = proxy_str.split("@");

  if (proxy_obj.socksHost.length > 1) {
    proxy_obj.socksUsername = proxy_obj.socksHost[0].split(":");
    if (proxy_obj.socksUsername.length > 1) {
      proxy_obj.socksPassword = proxy_obj.socksUsername[1];
      proxy_obj.socksUsername = proxy_obj.socksUsername[0];
    } else {
      proxy_obj.socksUsername = proxy_obj.socksHost[0];
    }

    proxy_obj.socksHost = proxy_obj.socksHost[1].split(":");
  } else {
    proxy_obj.socksHost = proxy_str.split(":");
  }

  if (proxy_obj.socksHost.length > 1) {
    proxy_obj.socksPort = proxy_obj.socksHost[1];
    proxy_obj.socksHost = proxy_obj.socksHost[0];
  } else {
    proxy_obj.socksHost = proxy_obj.socksHost[0];
  }
  return proxy_obj;
...
exports.parseSocksUrl = parseSocksUrl;
