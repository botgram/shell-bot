...
Renderer.prototype._linesRemoving = function _linesRemoving(y, n) {
  ...
  while (n) {
    var lines = (idx === this.messages.length) ? this.orphanLines : this.messages[idx];
    if (!lines) { console.error("Oooops! Hit bug, trying to fix up", this.orphanLines, n, this.messages, this.state.lines); break; }
...
Renderer.prototype.render = function render(message) {
  ...
  var html = message.lines.map(function (line) {
    ...
    if (hasCursor) {
      ...
      if (content.trim().length) isWhitespace = false;
  ...
  if (isWhitespace) return "<em>(empty)</em>";  //FIXME: represent empty lines properly, instead
  return html;
};
