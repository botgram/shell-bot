...
Renderer.prototype.ensureLinesCreated = function ensureLinesCreated(y) {
  if (this.totalLines < y) {
    this.orphanLines = Array.concat(this.orphanLines, this.state.lines.slice(this.totalLines, y));
    this.totalLines = y;
  }
}
...
Renderer.prototype.emitMessage = function emitMessage(count, silent) {
  ...
  var lines = this.orphanLines.unshift(0, count);
...
Renderer.prototype.render = function render(message) {
  return message.lines.map(function (line) {
    ...
    if (hasCursor) {
      ...
      var cursorString = this.state.getMode("cursorBlink") ? options.cursorBlinkString : options.cursorString;
...
Renderer.prototype.orphanLinesUpdated = function orphanLinesUpdated() {
  var newLines = this.orphanLines.slice(this.orphanLines.length - 1);
  if (newLines.length >= this.options.maxLinesWait) {
    // Flush immediately
    this.flushNew();
  ...
  if (unfinishedLine.str.length) this.unfinishedLineTimer.cancel();
  else this.unfinishedLineTimer.set();
};
...
Renderer.prototype.flushNew = function flushNew() {
  this.flushEdited();
  if (this.orphanLines.length <= 1) throw new Error("Should not happen.");
